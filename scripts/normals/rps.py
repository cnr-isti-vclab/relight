...
#from scripts import psutil
#from scripts import rpsnumerics
import psutil
import rpsnumerics
import numpy as np
...
class RPS(object):
    ...
    def load_lightlp(self, filename=None):
        """
            ...
            """
        self.L = psutil.load_lightlp(filename)
    ...
    def save_normalmap(self, filename=None):
        ...
        psutil.save_normalmap_as_tiff(filename=filename, normal=self.N, height=self.height, width=self.width)
        psutil.save_normalmap_as_png(filename=filename, normal=self.N, height=self.height, width=self.width)
        psutil.save_normalmap_as_jpg(filename=filename, normal=self.N, height=self.height, width=self.width)
        psutil.save_normalmap_as_json(filename=filename, normal=self.N, height=self.height, width=self.width)
    ...
    def solve(self, method=L2_SOLVER):
        ...
        if self.M.shape[1] != self.L.shape[1]:
            raise ValueError("Inconsistent dimensionality between M and L (number of images and number of coordinates in lights.txt)")
...
#            if index % 100 == 0:
            print(str(index) + "/" + str(self.M.shape[0]), flush=True)
