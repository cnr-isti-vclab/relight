...
void tiffZoom(QString inputFolder, QString output, uint32_t quality, uint32_t tileSize,
    ...
    QDir planeDir(inputFolder);
    QStringList planes = planeDir.entryList(QStringList(QString("plane_*.jpg")), QDir::Files, QDir::Name);
    
    if(planes.isEmpty()) {
        throw QString("No plane files found in %1").arg(inputFolder);
    }
    
    bool remove = true;
    
    // Convert each plane to a tiled TIFF pyramid
    for(int i = 0; i < planes.size(); i++) {
        QString planeFile = planeDir.absoluteFilePath(planes[i]);
        QString planeName = QString("plane_%1").arg(i);
        QString tiffOutput = planeDir.absoluteFilePath(planeName);
        
        DeepZoom dz;
        dz.quality = quality;
        if(!dz.build(planeFile, tiffOutput, tileSize, 0, PyramidFormat::TiledTiff)) {
            throw QString("Failed to build TIFF pyramid for %1").arg(planes[i]);
        }
        
        // Remove the original JPEG after successful TIFF creation
        if(remove)
            QFile::remove(planeFile);
        
        if(!progressed("Building IIP TIFF:", 100*(i+1)/planes.size()))
            break;
    }
}
...
void tarZoom(QString inputFolder, QString output, std::function<bool(QString s, int n)> progressed) {
    ...
    if (nPlanes == 0)
        throw QString("No dzi files in input folder %1").arg(inputFolder);
    ...
    for (int i=0; i<nPlanes; i++)
    {
        ...
        if (!outFile.open(QIODevice::WriteOnly))
            throw QString("Couldn't open output file %1").arg(outPath);
        if (!outIndexFile.open(QIODevice::WriteOnly))
            throw QString("Couldn't open output index file %1").arg(outIndexPath);
            ...
            for (size_t i = 0; i < orderedFiles.size(); i++)
            {
                ...
                if (file != nullptr)
                {
                    if (!file->open(QIODevice::ReadOnly))
                        throw QString("Couldn't open image file %1 for processing").arg(file->fileName());
...
void itarZoom(const QString& inputFolder, const QString& output, std::function<bool(QString s, int n)> progressed)
{
    ...
    if (nPlanes == 0)
        throw QString("No tzb files in input folder %1").arg(inputFolder);

    int nIndexes = getNFiles(inputFolder, "tzi");
    if (nIndexes == 0)
        throw QString("No tzi files in input folder %1").arg(inputFolder);
    if (nIndexes != nPlanes)
        throw QString("The number of .tzi and .tzb files doesn't match");
    ...
    for (int i=0; i<nPlanes; i++)
    {
        ...
        if (!files[i]->open(QIODevice::ReadOnly))
            throw QString("Error while opening .tzb file %1").arg(files[i]->fileName());
