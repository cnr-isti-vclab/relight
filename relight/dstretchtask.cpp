...
void DStretchTask::run()
{
    ...
    // Vector used to store pixel samples
    ...
    for (int row=0; row<height; row++)
    {
        ...
        for (int l=0; l<3; l++)
        {
            for (int m=0; m<3; m++)
            {
                unsigned long sumX = 0, sumM = 0, sumL = 0;
                for (int k=0; k<samples.size(); k++)
                {
                    ...
                    // Compute the means here to save a loop
                    if (m==0)
                        means(l) += samples[k][l];
                }
        ...
        int a = 10;
        // Save the pixels
        encoder.writeRows(pixels.data(), 1);

        progressed("DStretching...", (row * 100) / width);
        qDebug() << "Row: " << row;
    }
